function [out,trace,ons] = readpraatdata_YS(filename,reffreq,REC_DURATION, TIMESTEPS_PER_SECOND)
...
% Adapted by Yaqing Su 06.2024
% align pitch traces to the onset of the audio signal
...
tstart = round(ts(1)*TIMESTEPS_PER_SECOND);
trace = zeros(len+TIMESTEPS_PER_SECOND,1);
% trace(1:size(ts,1)) = f0;
% align all pitch traces relative to the onset of the audio signal
trace((tstart+1):(tstart+size(ts,1))) = f0;
...
if isempty(ons)
    ...
    align((1:min(length(temp),len))+ons(1)-1) = temp(1:min(length(temp),len));
    if length(align)>len
        align(len+1:end) = [];
    end
    align(align==0) = nan;
